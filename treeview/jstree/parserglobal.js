...
function buildTree(presentationData){
 ...
 function start(){
      ...
      //console.log(1+3);
      var json = JSON.parse(fs.readFileSync('global_data.json', 'utf8'));
      //console.log(json);
      forest= processData(json);
      var presentationData =[];
      ...
      for(var g=0; g< forest.trees.length; g++)
        ...
        var finalData = buildTree(presentationData);
          ...
          //console.log(content);
      fs.writeFile("global_parse.json", content, 'utf8', function (err) {
          if (err) {
...
function processData(data){
    ...
    var parent =  {
        text :  data[0].object +' ('+data[0].object_dbxref+')',
        parent : [],
        children : [],
        dbxref : data[0].object_dbxref,
        a_attr      : 
            {
                'href': '/chaise/record/#2/data_commons:cvterm/dbxref='+data[0].object_dbxref.replace(/:/g,'%3A'),
                'style': 'display:inline;'
            } 
    };
    var child =  {
        text :  data[0].subject +' ('+data[0].subject_dbxref+')',
        parent : [],
        children : [],
        dbxref : data[0].subject_dbxref,
        a_attr      : 
            {
                'href': '/chaise/record/#2/data_commons:cvterm/dbxref='+data[0].subject_dbxref.replace(/:/g,'%3A'),
                'style': 'display:inline;' 
            }
    };
    ...
    for(var i =1 ;i <data.length; i ++){
        ...
        //     console.log('found');
        var parent =  {
            text :  data[i].object +' ('+data[i].object_dbxref+')',
            parent : [],
            children : [],
            dbxref : data[i].object_dbxref,
            a_attr      : {'href': '/chaise/record/#2/data_commons:cvterm/dbxref='+data[i].object_dbxref.replace(/:/g,'%3A')} 
        };
        var child =  {
            text :  data[i].subject  +' ('+data[i].subject_dbxref+')',
            parent : [],
            children : [],
            dbxref : data[i].subject_dbxref,
            a_attr      : {'href': '/chaise/record/#2/data_commons:cvterm/dbxref='+data[i].subject_dbxref.replace(/:/g,'%3A')} 
        };
        var tree = new Tree(parent);
        var tree1 = new Tree(child);
        parent.children.push(child);
...
function Tree(node) {
    ...
    var s= node.a_attr;
    var l = "'/chaise/record/#2/data_commons:cvterm/dbxref="+linkId+"','_blank'";

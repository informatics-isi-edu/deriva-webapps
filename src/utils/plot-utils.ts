...
import { convertKeysSnakeToCamel } from './string';
...
export const validateLayout = (layouts: any, validatedUserControls: any, alertFunctions: any, global: boolean=false) => {
    ...
    tempLayout = Object.fromEntries(Object.entries(tempLayout).map(([layoutBPKey, layoutBPValue]) => {
      const newValue: any[] = (layoutBPValue as any[]).filter((val: any) => {
        if (!val?.sourceUid) {
          alertFunctions.addAlert(`Unable to display the ${global ? 'global' : 'local'} control due to a missing 'source_uid' in layouts parameter for "${layoutBPKey}" breakpoint`, ChaiseAlertType.WARNING);
          return false;
        }
        ...
        if (!(val?.w !== 0 && val?.w !== undefined && val?.h !== 0 && val?.h !== undefined && val?.x !== undefined && val?.y !== undefined)) {
          alertFunctions.addAlert(`Unable to display the ${global ? 'global' : 'local'} control with UID "${val.sourceUid}" due to a one of the required layout parameters missing(w, h, x, y) for "${layoutBPKey}" breakpoint`, ChaiseAlertType.WARNING);
